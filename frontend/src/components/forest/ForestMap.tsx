...
import { MapContainer, TileLayer, FeatureGroup, CircleMarker, Popup, GeoJSON } from 'react-leaflet';
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      >
        ...
        {/* Risk zone polygons */}
        {activeLayer === 'risk' && riskZones?.zones?.map((zone, i) => {
          const color = zone.risk === 'High' ? '#ef4444' : zone.risk === 'Moderate' ? '#f59e0b' : '#22c55e';
          return (
            <GeoJSON
              key={i}
            ...
            >
              <Popup><b>{zone.risk} Risk Zone</b><br />Detected degradation hotspot</Popup>
            </GeoJSON>
          );
        })}
        ...
        {riskAlerts.map(({ location, severity }, i) => {
          ...
          return (
            ...
            >
              <Popup>
                <b>{severity} Risk Alert</b><br />Deforestation / fire anomaly detected
              </Popup>
            </CircleMarker>
          );
        })}
      ...
      <div style={{
        ...
        padding: '8px 10px', boxShadow: '0 2px 10px rgba(0,0,0,0.15)',
        display: 'flex', flexDirection: 'column', gap: 4,
      }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: '#6b7280', marginBottom: 2, textTransform: 'uppercase' }}>Layers</div>
        {([
        ...
        {layerLoading && (
          <div style={{ fontSize: 10, color: '#6b7280', textAlign: 'center', paddingTop: 2 }}>Loading…</div>
        )}
      </div>
      ...
      <div style={{
        ...
        padding: '7px 12px', fontSize: 11, color: '#020f50',
        boxShadow: '0 2px 8px rgba(0,0,0,0.15)', maxWidth: 200, zIndex: 1000,
      }}>
        <strong>Draw a polygon</strong> over the Dang district forest to run instant analysis.
      </div>
    </div>
  );
}
