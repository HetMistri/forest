...
function fetchReducer(_state: FetchState, action: FetchAction): FetchState {
  switch (action.type) {
    ...
    case 'success': return { status: 'success', page: action.page };
    case 'error':   return { status: 'error', message: action.message };
  }
}

const PageRenderer: React.FC<PageRendererProps> = ({ fileUrl, spaPathname, extraBodyClass }) => {
  ...
  useEffect(() => {
    ...
    Object.entries(bodyAttrs).forEach(([key, value]) => {
      ...
      if (key === 'class') {
        document.body.className = value + (extraBodyClass ? ' ' + extraBodyClass : '');
      } else {
    ...
    if (page.headInlineStyles) {
      ...
      if (!document.getElementById(styleId)) {
        ...
        styleEl.id = styleId;
        styleEl.textContent = page.headInlineStyles.replace(/<\/?style[^>]*>/gi, '');
